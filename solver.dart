...
class Solution {
  ...
  @override
  String toString() {
    String _reversedWord = word.split('').reversed.join('');
    return 'Solution(word: $_reversedWord, startPos: $startPos, score: $score, isHorizontal: $isHorizontal, adjacentWords: $adjacentWords)';
  }
}

Set<Solution> assembleAllPossibleSolutions(
  GADDAG dictionary,
  Rack rack,
  Board board,
) {
  ...
  Set<String> _crossCheck((int, int) pos, bool isHorizontal) {
    ...
    while (board.isTileFilled(board.after(currentPos, isHorizontal))) {
      lettersBefore += board
          .getTile(board.after(currentPos, isHorizontal))
          .letter;
    ...
    while (board.isTileFilled(board.before(currentPos, isHorizontal))) {
      lettersAfter += board
          .getTile(board.before(currentPos, isHorizontal))
          .letter;
  ...
  _findAllPossibleWordsForAnchor(
    ...
    if (currentNode.isEndOfWord) {
      var (String originalWord, (int, int) startPos) = board
          .getOriginalWordAndStartPosFromEncoding(
            dictionary,
            anchorPos,
            string,
            isHorizontal,
      ...
      if (originalWord == 'אפוצ' && startPos == (4, 4)) {
        print('debug');
      }

      List<String> adjacentWords = board.getAdjacentWords(
        startPos,
        originalWord,
        isHorizontal,
      );
    ...
    if (board.isTileFilled(currentPos)) {
      ...
      if (currentNode.children.containsKey(boardLetter)) {
        if (arrivedDelimiter) {
          _findAllPossibleWordsForAnchor(
            string + boardLetter,
            currentNode.children[boardLetter]!,
            arrivedDelimiter,
            board.after(currentPos, isHorizontal),
            anchorPos,
            isHorizontal,
          );
        } else {
          _findAllPossibleWordsForAnchor(
            ...
            currentNode.children[boardLetter]!,
            arrivedDelimiter,
            board.before(currentPos, isHorizontal),
            anchorPos,
            isHorizontal,
          );
        }
    ...
    for (String letter in currentNode.children.keys) {
      if (rack.letters.contains(letter) &&
          board
              .getTile(currentPos)
              .validLetters
              .contains(
                letter,
              )) // _crossCheck(currentPos, !isHorizontal).contains(letter))
      {
        ...
        _findAllPossibleWordsForAnchor(
          ...
          currentNode.children[letter]!,
          true,
          board.after(anchorPos, isHorizontal),
          anchorPos,
          isHorizontal,
        );
      }
...
void main() async {
  ...
  Rack rack = Rack(7);
  // rack.letters = ['צ', 'א', 'י', 'פ', 'ת', 'ש', 'ו'];
  print('Rack letters: ${rack.letters}');

  Set<Solution> solutions = assembleAllPossibleSolutions(
    dictionary,
    rack,
    board,
  );
  ...
  List<String> result = board.getAdjacentWords(
    (1, 0),
    bestSolution!.word,
    false,
  );
  ...
  board.insertString(
    bestSolution.startPos,
    bestSolution.word,
    bestSolution.isHorizontal,
  );
